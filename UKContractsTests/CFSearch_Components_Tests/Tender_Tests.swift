...
final class Tender_Tests: XCTestCase {
  ...
  func test_NoTenderPeriod() throws {
    
    let data = Data(dataStrNoTenderPeriod.utf8)
    
    let sut = try! TestHelpers.decode(Tender.self,from: data)
    
    XCTAssertEqual(sut.tenderPeriod?.formattedEndDate, "")
  }
  
  func test_NoPublishedDateAndNoStatus() throws {
    
    let data = Data(dataStrNoPublishedDate.utf8)
    
    let sut = try! TestHelpers.decode(Tender.self,from: data)
    
    XCTAssertEqual(sut.formattedDatePublished, "")
    XCTAssertEqual(sut.fStatus, "")
  ...
  func test_Decode() throws {
    ...
    do {
      ...
      XCTAssertEqual(sut.fStatus, "Complete")
      ...
      XCTAssertEqual(sut.formattedDatePublished, "Thu 19 May 2022, 23:59")
